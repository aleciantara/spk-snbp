...
class ImportRaporSiswa implements ToModel, WithStartRow, WithHeadingRow
{
    ...
    private function storeRapor($semester, $nisn, $row)
    {
        ...
        $rapor = Rapor::where('nisn', $nisn)
                        ->get();
                    

        $arrayCount = count($rapor);

        // $this->storeNilaiAgama($semester, $nisn, $row, $rapor);
        ...
        for ($index = 0; $index < $arrayCount; $index++) {
            ...
            $raporData = [
                'nisn' => $nisn,
                'pelajaran'     => $rapor[$index]['pelajaran'] ?? null,
                'sem_1_nilai_p' => $rapor[$index]['sem_1_nilai_p'] ?? null,
                'sem_2_nilai_p' => $rapor[$index]['sem_2_nilai_p'] ?? null,
                'sem_3_nilai_p' => $rapor[$index]['sem_3_nilai_p'] ?? null,
                'sem_4_nilai_p' => $rapor[$index]['sem_4_nilai_p'] ?? null,
                'sem_5_nilai_p' => $rapor[$index]['sem_5_nilai_p'] ?? null,
            ];
            ...
            if($index == 0){
                ...
                $raporData['sem_' . $semester . '_nilai_p'] = $row[($index + 7)];
            }
    ...
    private function storeNilaiAgama($semester, $nisn, $row, $rapor)
    {
        $raporData = [
            'nisn' => $nisn,
            'pelajaran'     => $rapor[0]['pelajaran'] ?? null,
            'sem_1_nilai_p' => $rapor[0]['sem_1_nilai_p'] ?? null,
            'sem_2_nilai_p' => $rapor[0]['sem_2_nilai_p'] ?? null,
            'sem_3_nilai_p' => $rapor[0]['sem_3_nilai_p'] ?? null,
            'sem_4_nilai_p' => $rapor[0]['sem_4_nilai_p'] ?? null,
            'sem_5_nilai_p' => $rapor[0]['sem_5_nilai_p'] ?? null,
        ];

        $nilaiAgama = $this->getNilaiAgama($row);

        $raporData['sem_' . $semester . '_nilai_p'] = $nilaiAgama;
