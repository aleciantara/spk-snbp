            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-header-action">
                        <a href="{{ route('siswa.edit', ['nisn' => $siswa->nisn]) }}" class="btn btn-primary">Detail Siswa</a>
                    </div>
                </div>
                ...
                <div class="card-body pb-0">
                    <div class="row m-4">
                        ...
                        <div class="row col-sm-12 pb-3">
                            <label class="col-sm-4 col-form-label font-weight-bold text-dark">Nama Lengkap: {{ $siswa->nama }}</label>
                            <label class="col-sm-4 col-form-label font-weight-bold text-dark">NISN: {{ $siswa->nisn }}</label>
                        </div>
                        ...
                        <div class="container p-0">
                            <div class="form-group row col-sm-12">
                                <input style="width: 280px; height: 27px" type="text" name="pelajaran[]" id="pelajaran" class="mr-4 form-control-sm form-control" value="{{ old('pelajaran.' . $index, $rapor->pelajaran) }}" placeholder="{{ $rapor->pelajaran }}" readonly>

                                @for ($i = 1; $i <= 5; $i++)
                                <input style="width: 36px; height: 27px" type="text" name="sem_{{ $i }}_nilai_p[]" class="form-control-sm form-control ml-6" placeholder="P" value="{{ old("sem_${i}_nilai_p.${index}", $rapor["sem_${i}_nilai_p"]) }}">
                                <label style="font-size:13px" class="mr-1 ml-1 col-form-label"> / </label>
                                <input style="width: 36px; height: 27px" type="text" name="sem_{{ $i }}_nilai_k[]" class="form-control-sm form-control mr-3" placeholder="K" value="{{ old("sem_${i}_nilai_k.${index}", $rapor["sem_${i}_nilai_k"]) }}">
                                @endfor
                                ...
                                <label style="font-size:13px" class="mr-1 ml-1 col-form-label"> / </label>
                                <input style="width: 43px; height: 27px" type="text" name="rata_nilai_k[]" class="form-control-sm  form-control " placeholder="{{ old("rata_nilai_k.${index}", $rapor["rata_nilai_k"]) }}" readonly>
                            </div>
                        </div>
                        @endforeach
